...
class CrmLead(models.Model):
    ...
    order_ids = fields.One2many('sale.quotation', 'opportunity_id', string='Orders')

    @api.depends('order_ids.state', 'order_ids.currency_id', 'order_ids.amount_untaxed', 'order_ids.create_date', 'order_ids.company_id')
    @api.depends('order_ids.state', 'order_ids.currency_id', 'order_ids.amount_untaxed', 'order_ids.create_date', 'order_ids.company_id')
    ...
    def action_view_sale_quotation(self):
        self.ensure_one()
        action = self.env["ir.actions.actions"]._for_xml_id("sale.action_orders_quotations")
        action['context'] = self._prepare_opportunity_quotation_context()
        ...
        if len(quotations) == 1:
            action['views'] = [(self.env.ref('sale.view_quotation_form').id, 'form')]
