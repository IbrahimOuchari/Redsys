...
class SaleOrder(models.Model):
    ...
    quotation_seq = fields.Char(string='Quotation Reference', readonly=True)
    ...
    def _compute_partner_credit_warning(self):
        for order in self:
            ...
            show_warning = order.state in ('draft', 'sale') and \
        ...
        for order in self:
            ...
            if invalid_companies:
                ...
                raise ValidationError(_(
                    "Your order contains products from company %(product_company)s whereas your order belongs to company %(quote_company)s. \n Please change the company of your quotation or remove the products from other companies (%(bad_products)s).",
    ...
    def _onchange_company_id_warning(self):
        ...
        if self.order_line and self.state == 'draft':
            return {
                'warning': {
                    'title': _("Warning for the change of your order's company"),
                    'message': _("Changing the company of an existing order might need some "
    ...
    def _get_portal_return_action(self):
        ...
        self.ensure_one()
        return self.env.ref('sale.action_orders')
