...
class PurchaseOrder(models.Model):
    ...
    rfq_seq = fields.Char(string="RFQ Reference", copy=False, readonly=False,)
    partner_ref = fields.Char('Vendor Reference', copy=False,
        help="Reference of the sales order or bid sent by the vendor. "
             "It's used to do the matching when you receive the "
             "products as this reference is usually written on the "
             "delivery order sent by your vendor.")
    date_order = fields.Datetime('Date Order', required=True, index=True, copy=False, default=fields.Datetime.now,
        help="Depicts the date within which the Quotation should be confirmed and converted into a purchase order.")
    date_approve = fields.Datetime('Confirmation Date', readonly=True, index=True, copy=False)
    partner_id = fields.Many2one('res.partner', string='Vendor', required=True, change_default=True, tracking=True, check_company=True, domain=[('is_supplier', '=', True)], help="You can find a vendor by its Name, TIN, Email or Internal Reference.")
    ...
    # def action_rfq_send(self):
    #     '''
    #     Cette fonction ouvre une fenêtre pour composer un email, avec le modèle d'achat EDI chargé par défaut.
    #     Elle met également à jour l'état du bon de commande à 'sent' si l'état actuel est 'draft'.
    #     '''
    #     self.ensure_one()
    #     ir_model_data = self.env['ir.model.data']
    #     try:
    ...
    #         ctx['model_description'] = _('Purchase Order')
    #
    #     # Mettre à jour l'état du bon de commande à 'sent' si l'état actuel est 'draft'
    #     if self.state == 'draft':
    ...
    def action_rfq_send(self):
        '''
        Cette fonction ouvre une fenêtre pour composer un email, avec le modèle d'achat EDI chargé par défaut.
        Elle met également à jour l'état du bon de commande à 'sent' après l'envoi par email.
        '''
        self.ensure_one()
        ir_model_data = self.env['ir.model.data']
        try:
        ...
        # Mettre à jour l'état du bon de commande à 'sent'
        self.write({'state': 'sent'})
    ...
    def action_create_invoice(self):
        ...
        sequence = 10
        for order in self:
            if order.invoice_status != 'to invoice' and not self._context.get('create_bill', False):
                continue
    ...
    def retrieve_dashboard(self):
        ...
        result['all_waiting'] = po.search_count([('state', '=', 'draft'), ('date_order', '>=', fields.Datetime.now())])
        result['my_waiting'] = po.search_count([('state', '=', 'draft'), ('date_order', '>=', fields.Datetime.now()), ('user_id', '=', self.env.uid)])
        result['all_late'] = po.search_count([('state', 'in', ['draft', 'to approve']), ('date_order', '<', fields.Datetime.now())])
        result['my_late'] = po.search_count([('state', 'in', ['draft', 'to approve']), ('date_order', '<', fields.Datetime.now()), ('user_id', '=', self.env.uid)])
    ...
    def confirm_reminder_mail(self, confirmed_date=False):
        for order in self:
            if order.state in ['purchase','sent','done'] and not order.mail_reminder_confirmed:
    ...
    def _confirm_reception_mail(self):
        for order in self:
            if order.state in ['purchase','sent','done'] and not order.mail_reception_confirmed:
